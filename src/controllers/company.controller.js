 ...
 * @param {Object} res - Express response object
 */
const getCompaniesByAgreementStatus = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: `Companies with ${status} agreement status retrieved successfully`,
      data: { companies }
    });
  } catch (error) {
 ...
 * @param {Object} res - Express response object
 */
const getCompanyPOCs = async (req, res) => {
  try {
    const { id: companyId } = req.params;
